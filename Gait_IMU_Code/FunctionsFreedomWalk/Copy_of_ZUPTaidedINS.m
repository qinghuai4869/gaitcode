...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [x_h,cov,a,vel_zeroerror_r,vel_zeroerror_l] = Copy_of_ZUPTaidedINS(u, zupt,head_angle)
...
[x_h_r(1:9,1),quat_r] = init_Nav_eq(u(1:6,:),simdata.init_heading1,simdata.init_pos1);  % Subfunction located further down in the file.
[x_h_l(1:9,1),quat_l] = init_Nav_eq(u(7:12,:),simdata.init_heading2,simdata.init_pos2);  % Subfunction located further down in the file.

last_update = 0;

% avg_a_l = [0 0 0]';avg_a_l
% avg_a_r = [0 0 0]';
a_r = zeros(3,N);
a_l = zeros(3,N);
vel_zeroerror_r = zeros(9,N);
vel_zeroerror_l = zeros(9,N);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k = 2:N
    ...
    % 匹配对应的角度修正
    if k==1072
    [x_h_r(:,k),quat_r,a_r(:,k)]=Navigation_equations_anglecopy(x_h_r(:,k-1),u_r,quat_r,head_angle(1)); % Subfunction located further down in the file.
    [x_h_l(:,k),quat_l,a_l(:,k)]=Navigation_equations_anglecopy(x_h_l(:,k-1),u_l,quat_l,head_angle(2)); % Subfunction located further down in the file.
    else
    [x_h_r(:,k),quat_r,a_r(:,k)]=Navigation_equations(x_h_r(:,k-1),u_r,quat_r); % Subfunction located further down in the file.
    [x_h_l(:,k),quat_l,a_l(:,k)]=Navigation_equations(x_h_l(:,k-1),u_l,quat_l); % Subfunction located further down in the file.
    end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if zupt_r(k) == false                
        [x_h_r(1:9,k),P_r,quat_r,vel_zeroerror_r(:,k)] = MeasurementUpdate(vel_pseudo_measurement, H_v, x_h_r(1:9,k), P_r, quat_r, R_v);
%         [x_h_r(1:9,k),P_r,quat_r,avg_a_r] = MeasurementUpdate(vel_pseudo_measurement, H_v, x_h_r(1:9,k), P_r, quat_r, R_v,avg_a_r,a_r(:,k));
        cov_r(:,k) = diag(P_r);
%        vel_error_r = mean(vel_zeroerror_r(4:6,:),2);
%     else
%         vel_error = (vel_error_r + vel_error_l)./2;
%         [x_h_r(1:9,k)] = MeasurementUpdate1(vel_error_r, H_v, x_h_r(1:9,k), P_r, quat_r, R_v);
        
    end
    if zupt_l(k) == false
        [x_h_l(1:9,k),P_l,quat_l,vel_zeroerror_l(:,k)] = MeasurementUpdate(vel_pseudo_measurement, H_v, x_h_l(1:9,k), P_l, quat_l, R_v);
%         [x_h_l(1:9,k),P_l,quat_l,avg_a_l] = MeasurementUpdate(vel_pseudo_measurement, H_v, x_h_l(1:9,k), P_l, quat_l, R_v,avg_a_l,a_l(:,k));
        cov_l(:,k) = diag(P_l);
%         vel_error_l = mean(vel_zeroerror_l(4:6,:),2);
%     else
%         vel_error = (vel_error_r + vel_error_l)./2;
%         [x_h_l(1:9,k)] = MeasurementUpdate1(vel_error_l, H_v, x_h_l(1:9,k), P_l, quat_l, R_v);
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [x, quat]=init_Nav_eq(u,heading,init_pos)
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [y,q,f_t]=Navigation_equations(x,u,q)
...
% error_vy = p(2,2);
% error_vz = p(3,3);
% error_v = [error_vx;error_vy;error_vz];



% if v~=0
%     x(4:6) = x(4:6) + acc_t * Ts - avg_t* Ts; 
%     y(1:3) = x(1:3) + (y(4:6) + x(4:6)) * Ts * 0.5;
% end
...
%%
function [y,q,f_t]=Navigation_equations_anglecopy(x,u,q,angle)
...
% 触发条件，目前按照时间看看
%后面再找合适的位置
% 1. 从当前更新后的四元数中，提取当前的滚转（Roll）和俯仰（Pitch）（复用高精度姿态）
Rb2t_temp = q2dcm(q);  % 陀螺仪更新后的临时DCM
current_roll = atan2(Rb2t_temp(3,2), Rb2t_temp(3,3));    % 当前滚转（不变）
current_pitch = -atan(Rb2t_temp(3,1)/sqrt(1-Rb2t_temp(3,1)^2));  % 当前俯仰（不变）

% 2. 输入修正后的航向角（Yaw_Corrected，需确保与原欧拉角定义一致）
% 注意：Yaw_Corrected的单位是弧度（rad），若为角度需先转弧度：Yaw_Corrected = deg2rad(yaw_corrected_deg)
Yaw_Corrected = y(9)-angle;  % 替换为你的修正航向角（如来自GPS、磁力计校准结果）

% 3. 基于「当前Roll + 当前Pitch + 修正Yaw」重新计算目标四元数q_corrected
% 3-2-1欧拉角转四元数公式（与原函数姿态定义匹配）
cr = cos(current_roll/2);    sr = sin(current_roll/2);
cp = cos(current_pitch/2);   sp = sin(current_pitch/2);
cy = cos(Yaw_Corrected/2);   sy = sin(Yaw_Corrected/2);

q_corrected(1) = cr*cp*cy + sr*sp*sy;  % q0（实部）
q_corrected(2) = sr*cp*cy - cr*sp*sy;  % q1（滚转分量）
q_corrected(3) = cr*sp*cy + sr*cp*sy;  % q2（俯仰分量）
q_corrected(4) = cr*cp*sy - sr*sp*cy;  % q3（偏航分量）
...
% 后续继续执行原函数的「模块3（四元数→欧拉角）」和「模块4（位置/速度更新）」
Rb2t=q2dcm(q);  % 此时q已包含修正航向，DCM和欧拉角都会同步修正
%*************************************************************************%
% Update position and velocity states using the measured specific force,
% and the newly calculated attitude.
%*************************************************************************%
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [x_h, P, quat,dx]=MeasurementUpdate(pseudo_measurement, H, x_h, P, quat, R)
