function [stepR,stepL,stepWidthR,stepWidthL] = fun_stepsLength2(pos_right,pos_left,steps_r,steps_L)
...
if v > 0
    stepR = abs(stepR1);
    stepL = abs(stepL1);
    stepWidthL= abs(stepWidthL1);
    stepWidthR= abs(stepWidthR1);

else 
    if v < 0
        stepR = abs(stepL1);
        stepL = abs(stepR1);
        stepWidthR= abs(stepWidthL1);
        stepWidthL= abs(stepWidthR1);
    else
        disp('input error \n please retype ')
    end
end
