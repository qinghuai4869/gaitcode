...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [x_h,cov,a,vel_zeroerror_r,vel_zeroerror_l,d_h] = ZUPTaidedINS(u, zupt)
...
%     [x_h_r(:,k),quat_r,a_r(:,k)]=Navigation_equations(x_h_r(:,k-1),u_r,quat_r,avg_a_r); % Subfunction located further down in the file.
%     [x_h_l(:,k),quat_l,a_l(:,k)]=Navigation_equations(x_h_l(:,k-1),u_l,quat_l,avg_a_l); % Subfunction located further down in the file.
...
%         [x_h_l(1:9,k)] = MeasurementUpdate1(vel_error_l, H_v, x_h_l(1:9,k), P_l, quat_l, R_v);
    ...
    d_h(k) = norm(x_h_r(1:3,k)-x_h_l(1:3,k));
    if strcmp(simdata.Centroid_Method,'on') && d_h(k)>simdata.range_constraint && (k-last_update)>simdata.min_rud_sep
...
function [x_h, P, quat]=MeasurementUpdate1(measurement_v,predict_v,H, x_h, P, quat, R)

% Calculate the Kalman filter gain
K = (P*H')/(H*P*H'+R);

% Calculate the prediction error.
z = measurement_v - predict_v;
K1 = K(4:6,:);
% Estimation of the perturbations in the estimated navigation states
dx = K1*z;

% estim_x(1:3) = [0,0,0]';
estim_x = predict_v + dx;
% estim_x(7:9) = [0,0,0]';
dest_x = K*estim_x;
% Correct the navigation state using the estimated perturbations. 
% [x_h(1:9), quat] = comp_internal_states(x_h(1:9), dest_x, quat);     % Subfunction located further down in the file.
x_h(1:9) = x_h(1:9)  +  dest_x;      
% Update the filter state covariance matrix P.
Id = eye(size(P));
P = (Id - K*H)*P;
